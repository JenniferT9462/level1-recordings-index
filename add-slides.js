#!/usr/bin/env node
// Attaches a Google Slides link to a day's entry in recordings.json,
...
// Usage:
//   node add-slides.js
// Then follow the prompts: pick a recording, paste the slides link.
...
async function main() {
  ...
  if (!Array.isArray(recordings) || recordings.length === 0) {
    console.error("recordings.json has no entries yet — nothing to attach slides to.");
    process.exit(1);
  }
  ...
  console.log("\nWhich recording should get a slides link?\n");
  sorted.forEach((rec, i) => {
    const already = rec.slides ? "  [already has slides]" : "";
  ...
  let slidesUrl = (await rl.question("\nPaste the Google Slides share link: ")).trim();
  rl.close();
  ...
  original.slides = slidesUrl;

  fs.writeFileSync(JSON_PATH, JSON.stringify(recordings, null, 2) + "\n", "utf8");
  console.log(`\nUpdated "${target.topic || target.id}" with the slides link.`);

  try {
    run("git add recordings.json");
    const label = target.topic || target.id;
    run(`git commit -m "Add slides link for ${label.replace(/"/g, "'")}"`);
    run("git push");
    console.log("Committed and pushed. Students will see the slides link within a few minutes.");
  } catch (err) {
